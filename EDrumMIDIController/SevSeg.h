...
public:
  ...
  void Begin(boolean mode_in, byte numOfDigits, byte digit1, byte digit2,
    byte segment1, byte segment2, byte segment3, byte segment4, byte segment5,
    byte segment6, byte segment7, byte segmentDP);

private:
  void displayCharacter(byte characterToDisplay);
  void SplitNumber(int);

  boolean mode, DigitOn, DigitOff, SegOn, SegOff;

  byte digit1, digit2;
  byte segmentA, segmentB, segmentC, segmentD, segmentE, segmentF, segmentG,
    segmentDP;
